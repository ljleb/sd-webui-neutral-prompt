...
- saliency-aware noise blending, using the `AND_SALT` keyword (credits to [Magic Fusion](https://magicfusion.github.io/) for the algorithm used to determine SNB maps from epsilons)
...
saliency-aware noise blending has been implemented using the `AND_SALT` prompt keyword, which stands for "`AND` SALienT". In one sentence, `AND_SALT` keeps track of high noise activity during denoising and takes complete ownership over any high-activation regions in the output.

Here's another way to look at it: let's say that the noise generated by the `AND` prompts is a nation, and that the noise(s) generated by `AND_SALT` prompts is a (are) neighbouring country(ies). You can then think of all these lands as fighting for shared territory: whoever strikes the strongest at any given time and location owns the place.
